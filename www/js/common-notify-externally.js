...
function showNotification(data)
{
	var user_data=JSON.parse(window.localStorage['userdata']);
	data.success = true;
	data.senders = new Array();
	data.senders[0] = {"n_id":1};
	data.senders[1] = {"n_id":2};
	if ( device.platform == 'android' || device.platform == 'Android'){
		alert("device detected");
		alert(JSON.stringify(data));
		alert("type: "+window.localStorage['user_type']+typeof(data.success) + data.success);
		alert("satisfy: "+(window.localStorage['user_type'] == 'user_sitter' && (typeof(data.success) != "undefinded" && data.success)));
		if(window.localStorage['user_type'] == 'user_sitter' 
		&& (typeof(data.success) != "undefinded" && data.success))
		{
			var gcm = require('node-gcm');
			var message = new gcm.Message();
			 
			//API Server Key
			alert("user sitter notif"+data.senders.length);
			for(i = 0; i<data.senders.length; i++)
			{ 
				// Value the payload data to send...
				var sender = new gcm.Sender('AIzaSyA9WevCKBlRZi6mslT0XfGZgfJFBgs1AYQ');
				var registrationIds = [];
				message.addData('message',"SOS sold by"+data.senders[i].n_id);
				message.addData('title','Push Notification Sample'+data.senders[i].n_id);
				message.addData('msgcnt','SOS cnt'+data.senders[i].n_id); // Shows up in the notification in the status bar in app
				message.collapseKey = 'col key'+data.senders[i].n_id;
				message.delayWhileIdle = true; //Default is false
				message.timeToLive = 3000;// Duration in seconds to hold in GCM and retry before timing out. Default 4 weeks (2,419,200 seconds) if not specified.
				 
				// At least one reg id required
				alert("push gcm");
				registrationIds.push(gregId);
				/**
